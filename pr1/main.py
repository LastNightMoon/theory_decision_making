...
rm = set()
metrics = {
    "Цена (100 тыс руб.)": (lambda x, y: x <= y, [*range(20, 33)], 33),  # чем меньше, тем лучше
    "Мощность (л.с.)": (lambda x, y: x >= y, [*range(200, 300)], 200),  # чем больше, тем лучше
    "Расход топлива (л/1000 км)": (lambda x, y: x <= y, [*range(75, 85)], 85),  # чем меньше, тем лучше
    "Объем багажника (л)": (lambda x, y: x >= y, [*range(500, 600)], 500),  # чем больше, тем лучше
}
